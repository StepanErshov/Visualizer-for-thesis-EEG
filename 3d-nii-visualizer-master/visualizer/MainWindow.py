...
class EEGPlotWidget(QtWidgets.QWidget):
    ...
    def load_data(self):
        ...
        self.eeg_data, self.eeg_times = self.data_from_raw_edf.get_data(
            return_times=True
        )
    ...
    def update_plot(self):
        ...
        selected_channel_data = self.eeg_data[
            channel_index, self.t_index_begin : self.t_index_end
        ]
        ...
        self.ax.plot(
            ...
            label=f"Канал: {self.data_from_raw_edf.ch_names[channel_index]}",
        )
        self.ax.set_title(
            f"Данные ЭЭГ для канала: {self.data_from_raw_edf.ch_names[channel_index]}"
        )
...
class PlotWidget(QtWidgets.QWidget):
    ...
    def plot_eeg(self):
        ...
        y = np.zeros(t_index_end - t_index_begin)
        for i in range(len(eeg_data)):
            y += eeg_data[i, t_index_begin:t_index_end]
        y /= len(eeg_data)

        self.ax.clear()
        self.ax.plot(t, y, label="Усредненный сигнал ЭЭГ")
        self.ax.set_title("Усредненный сигнал ЭЭГ")
        self.ax.set_xlabel("Время (с)")
        self.ax.set_ylabel("Амплитуда")
        self.ax.legend()
        self.ax.grid()
...
class MainWindow(QtWidgets.QMainWindow, QtWidgets.QApplication):
    def __init__(self, app):
        ...
        self.brain_slicer_props = setup_slicer(self.renderer, self.brain)
        self.slicer_widgets = []

        # brain pickers
        self.brain_threshold_sp = self.create_new_picker(
            self.brain.scalar_range[1],
            self.brain.scalar_range[0],
            5.0,
            sum(self.brain.scalar_range) / 2,
            self.brain_threshold_vc,
        )
    ...
    def add_spacer(self):
        # Create a spacer item to separate the 3D view and the plot
        spacer = QtWidgets.QWidget()
        spacer.setFixedHeight(30)  # Adjust height as needed
        self.grid.addWidget(spacer, 6, 6, 6, 6)  # Position the spacer in the grid

    def add_EGG_plot_widget(self):
        # Create and add the plot widget to the grid
        plot_widget = EEGPlotWidget()
        self.grid.addWidget(plot_widget, 6, 0, 6, 3)

    def add_plot_widget(self):
        plot_widget = PlotWidget()
        self.grid.addWidget(plot_widget, 6, 3, 6, 3)
    ...
    def add_vtk_window_widget(self):
        ...
        object_title = (
            "Brain: {0} (min: {1:.2f}, max: {2:.2f})        Mask: {3}".format(
                base_brain_file,
                self.brain.scalar_range[0],
                self.brain.scalar_range[1],
                base_mask_file,
    ...
    def add_brain_settings_widget(self):
        ...
        # order is important
        slicer_funcs = [
            self.axial_slice_changed,
            self.coronal_slice_changed,
            self.sagittal_slice_changed,
        ]
        current_label_row = 6
    ...
    def axial_slice_changed(self):
        ...
        self.brain_slicer_props[0].SetDisplayExtent(
            self.brain.extent[0],
            self.brain.extent[1],
            self.brain.extent[2],
            self.brain.extent[3],
    ...
    def coronal_slice_changed(self):
        ...
        self.brain_slicer_props[1].SetDisplayExtent(
            self.brain.extent[0],
            self.brain.extent[1],
            pos,
            pos,
    ...
    def sagittal_slice_changed(self):
        ...
        self.brain_slicer_props[2].SetDisplayExtent(
            pos,
            pos,
            self.brain.extent[2],
            self.brain.extent[3],
    ...
    def brain_projection_vc(self):
        ...
        self.brain_slicer_cb.setDisabled(
            projection_checked
    ...
    def brain_slicer_vc(self):
        ...
        self.brain_projection_cb.setDisabled(
            slicer_checked
